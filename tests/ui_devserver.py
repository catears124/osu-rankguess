#!/usr/bin/env python3
"""Serve public/ with mock osu!rankguess API responses for browser smoke tests."""
...
def guess_result(body: dict[str, object]) -> dict[str, object]:
    guess = int(body.get("guessRank", 1) or 1)
    attempt = int(body.get("attempt", 1) or 1)
    actual = ACTUAL.get(str(body.get("replayID")), 50_000)
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/auth/status":
            return self.send_json({"configured": False, "authenticated": False})
        return super().do_GET()
...
def main() -> None:
    ...
    print(f"mock osu!rankguess UI server: http://localhost:{port}", flush=True)
    try:
